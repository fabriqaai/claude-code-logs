...
func NewMarkdownGenerator(outputDir, sourceDir string, force bool, projects []Project) (*MarkdownGenerator, error) {
	...
	return &MarkdownGenerator{
		...
		shellTmpl:        shellTmpl,
		indexTmpl:        indexTmpl,
		projectIndexTmpl: projectIndexTmpl,
	}, nil
}
...
// GenerateSession generates a Markdown file and HTML shell for a single session
...
func GenerateAllMarkdown(projects []Project, outputDir, sourceDir string, force bool) (*GenerationResult, error) {
	gen, err := NewMarkdownGenerator(outputDir, sourceDir, force, projects)
	if err != nil {
